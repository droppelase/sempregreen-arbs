...
app = Flask(__name__)
CORS(app)
socketio = SocketIO(app, cors_allowed_origins="*")  # Permite WebSockets
...
def fetch_and_emit_arbs():
    """Busca surebets e envia para o WebSocket"""
    while True:
        try:
...
# Iniciar a thread para buscar e enviar as surebets
thread = Thread(target=fetch_and_emit_arbs)
thread.daemon = True
thread.start()
